...
window.PoseEngine = (function () {
  ...
  function _onResults(results, canvasEl) {
    ...
    // Compute joint angles with responsive EMA smoothing to filter out camera noise
    const rawAngles = _exercise.computeAngles(lm);
    const angles = {};
    Object.entries(rawAngles).forEach(([k, v]) => {
      if (typeof v === 'number' && !isNaN(v)) {
        if (_smoothedAngles[k] !== undefined) {
          _smoothedAngles[k] = Math.round(_smoothedAngles[k] * 0.25 + v * 0.75);
        } else {
          _smoothedAngles[k] = v;
        }
        angles[k] = _smoothedAngles[k];
      }
    });
    _lastAngles = angles;
    ...
    // ── REP STATE MACHINE ──
    // 'ready' phase waits for user to be positioned or detects initial movement
    if (_phase === 'ready') {
  ...
  function _tryEnterFirstPhase(angles) {
    ...
    if (!_exercise.rom || Object.keys(_exercise.rom).length === 0) {
      _phase = 'up';
    ...
    const bottomAngle = romDef.bottom;
    const tolerance   = (_exercise.romTolerance || 15) + 20; // generous tolerance for start gate

    // Check if within generous start range of top
    ...
    // Also allow starting if user is closer to bottom bound or inverted ROM
    if (Math.abs(angle - bottomAngle) <= tolerance) {
      _phase = 'up';
      _topAngles = { ...angles };
      if (_callbacks.onPhaseChange) _callbacks.onPhaseChange('up');
      return;
    ...
    // If user has already started moving (e.g. state machine would register transition-down)
    if (_exercise.repStateMachine) {
      const wouldMove = _exercise.repStateMachine('up', angles, _exercise);
      if (wouldMove && wouldMove !== 'up') {
        _phase = 'up';
        _onPhaseChange('ready', 'up', angles);
        _phase = wouldMove;
        _onPhaseChange('up', wouldMove, angles);
        return;
      }
    }

    // Auto-advance after ~10 frames of stable pose detection so user is never stuck
    if (_readyFrameCounter > 10) {
      _phase = 'up';
      _topAngles = { ...angles };
      if (_callbacks.onPhaseChange) _callbacks.onPhaseChange('up');
    }
  ...
  function _onPhaseChange(from, to, angles) {
    ...
    // Rep completes when we return to 'up' and an inflection occurred
    if (to === 'up' && (_downAngles || from === 'down' || from === 'transition-up') && !_repFired) {
      _repFired = true; // lock until next descent

      const anglesForValidation = _downAngles || angles;
      const repResult = (_exercise.validateRep)
        ? _exercise.validateRep(anglesForValidation, angles, _exercise)
        : { ok: true, errors: [] };
    ...
    // Unlock for next rep cycle as soon as user begins descending/flexing
    if (to === 'down' || to === 'transition-down') {
      _repFired = false;
      if (to === 'down' || !_downAngles) {
        _downAngles = { ...angles };
      }
    }
  }
